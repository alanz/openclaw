...
export async function sendMessageDeltaChat(
  ...
  try {
    ...
    runtime.log?.(`[Delta.Chat] Sent message ${messageId} to ${to}`);
    ...
    runtime.error?.(`[Delta.Chat] Failed to send message: ${errorMessage}`);
    return {
      ok: false,
      error: errorMessage,
    };
...
export async function sendMediaDeltaChat(
  ...
  try {
    ...
    runtime.log?.(`[Delta.Chat] Downloading media from ${mediaUrl}`);
    ...
    const tempFilePath = `${expandedDataDir}/temp_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    try {
      ...
      runtime.log?.(`[Delta.Chat] Sent media message ${messageId} to ${to} (type: ${viewtype})`);
      updateDeltaChatRuntimeState({ lastOutboundAt: Date.now() });
    ...
    const runtime = getDeltaChatRuntime();
    const errorMessage = extractErrorMessage(err);
    runtime.error?.(`[Delta.Chat] Failed to send media: ${errorMessage}`);
    return {
      ok: false,
      error: errorMessage,
    };
