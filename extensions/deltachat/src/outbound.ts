...
export async function sendDeltaChatMessage(
  ...
  try {
    const runtime = getDeltaChatRuntime();
    ...
    runtime.log?.(`[Delta.Chat] sendDeltaChatMessage called with to="${to}"`);
    const parsed = parseDeltaChatTarget(to);
    runtime.log?.(`[Delta.Chat] Parsed target: kind="${parsed.kind}", to="${parsed.to}"`);
    let targetChatId: number | undefined;

    if (parsed.kind === "chat_id") {
      // Use the chat ID directly
      targetChatId = parseInt(parsed.to, 10);
      runtime.log?.(`[Delta.Chat] Using chatId directly: ${targetChatId}`);
    } else {
      // It's an email address, create or get the chat
      runtime.log?.(`[Delta.Chat] Creating chat for email: ${parsed.to}`);
      try {
        ...
        runtime.log?.(`[Delta.Chat] Created chat with ID: ${targetChatId}`);
      } catch (err) {
        return {
          ok: false,
    ...
    runtime.log?.(`[Delta.Chat] Sending message to chatId ${targetChatId}`);
    const messageId = await dc.rpc.miscSendTextMessage(dcAccount.id, targetChatId, text);

    runtime.log?.(`[Delta.Chat] Sent message ${messageId} to ${to} (chatId: ${targetChatId})`);
