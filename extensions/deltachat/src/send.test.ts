...
vi.mock("./targets.js", () => ({
  parseDeltaChatTarget: vi.fn((target) => {
    let normalized = target;
    if (typeof normalized === "string" && normalized.toLowerCase().startsWith("deltachat:")) {
      normalized = normalized.slice("deltachat:".length).trim();
    }
    if (typeof normalized === "number") {
      return { kind: "chat_id", to: String(normalized) };
    }
    if (typeof normalized === "string" && normalized.includes("@")) {
      return { kind: "email", to: normalized };
    }
    return { kind: "chat_id", to: normalized };
  }),
}));
...
describe("Delta.Chat Send - chunkText", () => {
  ...
  describe("Delta.Chat Send - deliverReplies", () => {
    ...
    beforeEach(async () => {
      ...
      vi.mocked(rpcServerManager.get).mockReturnValue(mockDc);
    ...
    it("should handle missing target", async () => {
      ...
      expect(mockRuntime.error).toHaveBeenCalledWith(
        "Failed to send reply to chatId 0: Invalid target",
      );
    });
...
describe("Delta.Chat Send - sendMessageDeltaChat", () => {
  ...
  it("should strip deltachat: prefix from target before creating contact", async () => {
    const { sendMessageDeltaChat } = await import("./send.js");
    ...
    const result = await sendMessageDeltaChat("deltachat:user@example.com", "Hello!", {
      cfg: mockConfig as any,
      accountId: "test",
    });

    expect(result.ok).toBe(true);
    // prefix must be stripped — contact must be created with plain email
    expect(mockDc.rpc.createContact).toHaveBeenCalledWith(
      "test",
      "user@example.com",
      "user@example.com",
