...
- **Liveness Reactions**: Cycles through reaction emojis (⏳ ⚙️ 🤔 💭) to show the bot is processing messages (since Delta.Chat doesn't support native typing indicators)
...
| `groups`                          | object  | Group allowlist configuration                         | `{}`        |
| `livenessReactionsEnabled`        | boolean | Enable/disable liveness reaction indicators           | `true`      |
| `livenessReactionIntervalSeconds` | number  | Interval (in seconds) between liveness reactions      | `15`        |
...
Since Delta.Chat doesn't support native typing indicators, the bot uses cycling reaction emojis to show it's processing messages. This feature is enabled by default.
...
```json
{
  "channels": {
    "deltachat": {
      "livenessReactionsEnabled": true,
      "livenessReactionIntervalSeconds": 15
    }
  }
}
```

The bot will cycle through reactions (⏳ → ⚙️ → 🤔 → 💭) every 15 seconds while processing a message. The reaction is automatically cleared when the response is sent.

To disable liveness reactions:

```json
{
  "channels": {
    "deltachat": {
      "livenessReactionsEnabled": false
    }
  }
}
```
...
- **Reactions**: Not supported by Delta.Chat core
- **Threads**: Delta.Chat doesn't have thread support
- **Editing**: Message editing not supported
- **Unsend**: Message unsending not supported
- **Replies**: Basic reply support via context
...
- [Alan's Guide](/Users/alanz/how_to_write_a_deltachat_bot_using_typescript.org)
